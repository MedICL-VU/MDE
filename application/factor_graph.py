...
class FactorGraph:
    def __init__(self, video, update_op, device="cuda:0", corr_impl="volume", max_factors=-1, upsample=False):
        ...
        # self.flows = None # torch.zeros([1, 0, ht, wd, 2], device=device, dtype=torch.float)
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if use_inactive:
                ...
                target_only_ac = self.target
                weight_only_ac = self.weight
                target = torch.cat([self.target_inac[:,m], self.target], 1)
                weight = torch.cat([self.weight_inac[:,m], self.weight], 1)
                # print(f"Weight after use_inactive: {weight.shape}")
            ...
            # print("\n\nFactor Graph Update")
            # # self.video.upsample(torch.unique(self.ii), upmask)
            # print(self.video.disps[-1].shape)
            # print(self.video.disps[-1].unsqueeze(-1).shape)
            # print(f"delta shape, {delta.shape}")
            # print(f"self.weight.shape, {self.weight.shape}")
            # print(f"weight.shape, {weight.shape}")
            # # print(weight.unsqueeze(-1).shape)
            # print(f"ii shape, {ii.shape}")
            # print(f"self ii shape, {self.ii.shape}")
            # print(f"unique ii, {torch.unique(self.ii).shape}")
            # print(self.ii)
            # print(f"jj shape, {jj.shape}")
            # print(f"unique jj, {torch.unique(self.jj).shape}")
            # print(self.jj)
            # print(f"Up Mask Shape: {upmask.shape}")
            # # print(f"")
            # # self.video.upsample(torch.unique(self.ii), upmask)
            
            
            
            all_nodes = torch.unique(torch.cat([self.ii, self.jj]))
            largest_nodes_vals, largest_nodes_indices = torch.topk(all_nodes, 2)
            node1 = largest_nodes_vals[0]  # largest
            node2 = largest_nodes_vals[1]  # second largest
            # print(f"Largest node: {largest_nodes_vals}; second largest node: {second_largest_node}")
            edge_condition = ((self.ii == node1) & (self.jj == node2)) | ((self.ii == node2) & (self.jj == node1))
            # print(edge_condition)
            edge_indices = torch.nonzero(edge_condition).flatten()
            # torch.set_printoptions(threshold=float('inf'))
            # print(edge_indices)
            edge_index = edge_indices[0]
            
            # latest_weight = self.weight[]
            # weight_upsample = cvx_upsample(self.video.disps[torch.unique(self.ii)].unsqueeze(-1), upmask)
            # print(f"Cat Weight Shape {self.weight[0:1, edge_index, :, :, 0].unsqueeze(-1).repeat(upmask.shape[1], 1, 1, 1).shape}")
            weight_upsample = cvx_upsample(self.weight[0:1, edge_index, :, :, 0].unsqueeze(-1).repeat(upmask.shape[1], 1, 1, 1), upmask)
            weight_upsample = weight_upsample[-1, :, :, 0] # TODO: -1, :, :, :, 
            self.video.confidence_maps[self.video.counter.value] = weight_upsample.cpu()
            # print(f"Weight Upsampled Shape: {weight_upsample.shape}")
        self.age += 1
    ...
    def update_lowmem(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, steps=8):
        ...
        for step in range(steps):
            ...
            for i in range(0, self.jj.max()+1, s):
                v = (self.ii >= i) & (self.ii < i + s) # Iterate through the graph in increment of 8 edges?
                iis = self.ii[v]
                jjs = self.jj[v]

                ht, wd = self.coords0.shape[0:2]
                corr1 = corr_op(coords1[:,v], rig * iis, rig * jjs + (iis == jjs).long()) # IF ii=jj, or an edge to self, change edge to be to the next frame?
                ...
                # self.flows[:, v] = delta.float()
                self.weight[:, v] = weight.float()
                self.damping[torch.unique(iis)] = damping
                
                # print(f"self.delta shape: {self.delta.shape}; motn shape: {motn.shape}")
                # print(f"self.weight shape: {self.weight.shape}, self.ii shape: {self.ii.shape}, self.jj shape: {self.jj.shape}")
                # print(self.ii)
                # print(self.jj)
            ...
            # self.flows = target.float()
            # self.weights = weight.float()
            # print(f"target shape: {target.shape}; motn shape: {motn.shape}")
            # print(f"weight shape: {weight.shape}, self.ii shape: {self.ii.shape}, self.jj shape: {self.jj.shape}")
                
        return self.ii, self.jj, self.target, self.weight
