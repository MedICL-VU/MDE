...
class MotionFilter:
    ...
    def track(self, tstamp, image, depth=None, intrinsics=None):
        f = open("flow.txt", "a")
        t = open('tracked_frames.txt', "a")
        """ main update operation - run on every frame in video """
        Id = lietorch.SE3.Identity(1,).data.squeeze()
        ht = image.shape[-2] // 8
        wd = image.shape[-1] // 8

        # print(f"size of the deep feature after condensing {image.shape}")
        ...
        ### only add new frame if there is enough motion ###
            ...
            # print('Flow: ', delta.norm(dim=-1).mean().item(),file=f)
            # print(f"Weight shape: {weight.shape}; min/max: {weight.min()}/{weight.max()}; mean: {weight.mean()}; std: {weight.std()}")
            # print(f"Weight shape: {delta.shape}; min/max: {delta.min()}/{delta.max()}; mean: {delta.mean()}; std: {delta.std()}")
