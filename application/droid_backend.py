...
class DroidBackend:
    ...
    def __call__(self, steps=12):
        ...
        ii, jj, flows, weight = graph.update_lowmem(steps=steps)
        self.ii, self.jj, self.flows, self.weight = ii.cpu(), jj.cpu(), flows.cpu(), weight.cpu() # todo: maybe not move to cpu so early
        graph.clear_edges()
