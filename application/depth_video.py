...
class DepthVideo:
    def __init__(self, image_size=[480, 640], buffer=1024, stereo=False, device="cuda:0"):
        ...
        self.poses = torch.zeros(buffer, 7, device="cuda", dtype=torch.float).share_memory_()
                # NEW: Allocate segmentation mask storage
        self.masks = torch.zeros(buffer, 1, ht, wd, device="cuda", dtype=torch.uint8)
        self.confidence_maps = torch.zeros(buffer, ht, wd, device="cuda", dtype=torch.float)
    ...
    def ba(self, target, weight, eta, ii, jj, t0=1, t1=None, itrs=2, lm=1e-4, ep=0.1, motion_only=False):
        ...
        with self.get_lock():
            ...
            # print(f"Depth video ba, shape of disps: {self.disps.shape}")
            # print(f"disps: {self.disps[-1].mean()}\n{self.disps[-1]}")
            # print(f"disps_sens: {self.disps_sens[-1].mean()}\n{self.disps_sens[-1]}")
            ...
            # print(f"after droid_backends ba, shape of disps: {self.disps.shape}")
            # print(f"disps: {self.disps[-1].mean()}\n{self.disps[-1]}")
            # print(f"disps_sens: {self.disps_sens[-1].mean()}\n{self.disps_sens[-1]}")
